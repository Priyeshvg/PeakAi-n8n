...
export class PeakAiPhone implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Peak AI Phone',
		name: 'peakAiPhone',
		icon: 'file:peakai.svg',
		group: ['transform'],
		version: 1,
		description: 'Get phone number from LinkedIn profile',
		defaults: {
			name: 'Peak AI Phone',
		},
		inputs: ['main'],
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				const response = await this.helpers.httpRequest({
					method: 'GET',
					url: `https://automation.sayf.in/webhook/extractor?access_token=${accessToken}&type=phone_no&profile_url=${encodeURIComponent(linkedinUrl)}`,
					json: true,
				});
				...
				returnData.push({
					json: {
						linkedinUrl,
						phoneNumber: response.phone_no?.[0]?.item || null,
					},
					pairedItem: { item: i },
				});
			} catch (error) {
